...
结合当前源码，这个项目最值得关注的功能特点是：

1. 它是一个“多模态分类模型”，不是生成式问答系统。Qwen 在这里承担的是高层表示建模器，而非直接文本生成器。
2. 视觉学习被刻意强化了。训练集文本遮蔽、图像增强和跨模态对齐都在服务这个目标。
3. HSI 是增强项，不是强依赖。没有 HSI 时，系统仍可在 `RGB + Text` 模式下运行。
4. 项目强调参数高效微调。Qwen 主体被冻结，LoRA 负责低成本适配。
5. 工程流程完整。数据准备、训练、评估、推理、可视化都已有脚本支撑。

## 14. 使用注意事项

根据当前代码，实际使用时要注意：

- 训练脚本要求 CUDA GPU；没有 GPU 时训练会直接报错。
- `--qwen_path` 必须指向本地可加载的 Qwen2.5-7B 模型目录。
- `class_mapping.json` 的实际类别数会覆盖默认配置。当前仓库示例映射为 15 类，而配置文件默认 `num_classes=50` 更像占位值。
- 如果没有真实 HSI，虽然代码支持 dummy HSI 或零张量占位，但最终性能会更接近 `RGB + Text` 模型，而非真正的光谱增强模型。
- 文本描述如果写得过于直接、等同于标签，会削弱模型评估的真实性；仓库已经通过随机描述和文本遮蔽来缓解这个问题。
...
.
├── config/         # 模型与训练配置
├── data/           # 数据集定义、预处理脚本、类别映射
├── models/         # 编码器、融合模块、适配器、主模型
├── scripts/        # 训练、评估、推理、测试集收集脚本
├── training/       # 通用 Trainer 和损失函数
├── utils/          # 指标与可视化工具
└── requirements.txt
```
...
如果你要快速理解代码，建议按下面顺序阅读：

1. [`models/main_model_qwen.py`]
2. [`data/dataset.py`]
3. [`models/fusion/cross_modal_alignment.py`]
4. [`models/fusion/multi_modal_fusion.py`]
5. [`models/adapters/embedding_adapter.py`]
6. [`scripts/train_qwen_fixed.py`]
